...
RUN apt-get update && apt-get install --no-install-recommends -y wget gnupg gnupg2 gnupg1 \
  ...
  && apt-get update \
  && apt-get install --no-install-recommends -y postgresql-client-10

# install php middleware
RUN apt-get update && apt-get install --no-install-recommends -y \
        git curl unzip vim wget sudo libfreetype6-dev libjpeg62-turbo-dev libmcrypt-dev libmcrypt-dev \
        libxml2-dev libpq-dev libpq5 postgresql-client mysql-client ssl-cert libicu-dev libmemcached-dev \
